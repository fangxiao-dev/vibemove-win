...
| 模式 | 氛围 | 场景 |
| --- | --- | --- |
| **`hand`**（默认） | 低调。坐在桌前动动手。 | 写代码、写文档、编辑——贴近键盘。 |
| **`body`** | 张扬。站起来，动起来。 | 站立办公桌、厨房、客厅——远离键盘。 |

```bash
swift run VibeMove              # hand 模式（默认）
...
cd vibemove
swift build
swift run VibeMove                       # hand 模式
swift run VibeMove -- --mode body        # body 模式
```
